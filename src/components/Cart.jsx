...
function Cart({ cart, setCart, cartOpen, setCartOpen }) {
  ...
  return (
    <div className={cartOpen ? 'absolute top-0 left-0 w-full h-full duration-300' : 'absolute top-[-110%] left-0 w-full h-full duration-300'}>
        <div className={cartOpen ? 'bg-black/50 fixed w-full h-screen z-10 top-0 left-0 cursor-pointer' : ''} onClick={() => setCartOpen(false)}></div>
        <div className='bg-white w-[90%] h-auto fixed z-10 ml-[5%] mt-[20%] p-2 md:mt-[10%] lg:mt-[5%] md:w-[50%] md:mx-[25%]'>
            <div className='flex text-3xl items-center justify-between shadow-xl'>
                <h1>Cart</h1>
                <AiOutlineClose className='cursor-pointer' onClick={() => setCartOpen(false)}/>
            </div>
                            ...
                            <div className='flex items-center justify-between mt-2 pr-2' key={index}>
                                ...
                                <div className='flex items-center'>
                                    ...
                                    <AiOutlineClose 
                                        ...
                                        onClick={() => setCart(removeFood(cart, item))}
                                    />
                                </div>
                            </div>
                        )
            ...
            <div className='flex justify-center mt-5'>
                <button 
                    ...
                    onClick={() => {alert(cart.length ? 'Good Job!' : 'No Food In Cart!'); setCartOpen(false)}}
                >Pay & Eat</button>
            </div>         
        </div>
    </div>
